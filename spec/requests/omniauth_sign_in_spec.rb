...
RSpec.describe 'OmniAuth sign in' do
  context 'request phase' do
    before(:each) { get '/api/v1/staff/auth/developer' }

    it 'displays a form' do
      get '/api/v1/staff/auth/developer'
      expect(response.status).to eq(200)
      expect(response.body).to be_include('<form')
    end

    it 'has the callback as the action for the form' do
      expect(response.body).to be_include("action='/api/v1/staff/auth/developer/callback'")
    end

    it 'has a text field for each of the fields' do
      expect(response.body.scan('<input').size).to eq(2)
    end
  end
